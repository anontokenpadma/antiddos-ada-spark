...
procedure Rideconnect is
   ...
   package body IO is
      ...
      begin
         ...
         Ada.Text_IO.Put_Line
           ("  RC_GLOBAL_RPS=300      max requests per second, total (all IPs)");
         Ada.Text_IO.Put_Line
      ...
      procedure Relay_Chunked
        (R : in out Reader; To : in out Client_Link;
         Max_Total : Natural; Ok, Too_Large : out Boolean)
      is
         Line : Chunk_Line_Buf;
         Ln   : SEO;
         St   : IO_Status;
      ...
      protected type Conn_Guard is
         procedure Try_Add_Total
           (Max_Total : Positive; Ok : out Boolean);
         procedure Try_Add_Per_IP
           (IP : Rate_Limiter.IP_Addr; Max_Per_IP : Positive; Ok : out Boolean);
         procedure Remove_Per_IP (IP : Rate_Limiter.IP_Addr);
         procedure Remove_Total;
      ...
      protected body Conn_Guard is
         ...
         procedure Remove_Per_IP (IP : Rate_Limiter.IP_Addr) is
      ...
      private
         ...
         Global_Bucket : Rate_Limiter.Bucket := (Active => False, others => <>);
      end Rate_Guard;
      ...
      protected body Rate_Guard is
         ...
         is
         begin
            Rate_Limiter.Consume_Global
              (Global_Bucket, Now_Seconds, Rate, Burst, Allowed);
         end Check_Global;
         ...
         is
         begin
            return Rate_Limiter.Global_Retry_After
              (Global_Bucket, Now_Seconds, Rate);
         end Global_Retry_After;
      ...
      begin
         ...
         case Resp.Frame_Kind is
            ...
            when Http_Parser.Frame_Chunked =>
               declare
                  Dummy_Too_Large : Boolean;
               begin
                  Relay_Chunked
                    (R, Client, Natural'Last / 2, Ok, Dummy_Too_Large);
               end;
      ...
      begin
         ...
         declare
            ...
            Body_Too_Large : Boolean := False;
         begin
            if Hdrs.Chunked then
               Relay_Chunked
                 (CR, Backend_Link, Cfg.Max_Body, Body_Ok, Body_Too_Large);
            elsif Hdrs.Has_Content_Length then
            ...
            if not Body_Ok then
               if Body_Too_Large then
                  ...
                  --  malformed chunk framing (bad/oversized size line)
      ...
      --  ---------- real client IP (behind cloudflared) ----------

      function Is_Loopback (IP : Rate_Limiter.IP_Addr) return Boolean is
      begin
         return IP (1) = Byte (127);
      end Is_Loopback;

      function Parse_IPv4
        (Buf : Head_Buffer; S, L : SEO; IP : out Rate_Limiter.IP_Addr)
         return Boolean
      is
         Oct : Natural := 0;
         Idx : Natural := 1;
      ...
      begin
         IP := [others => 0];
         while K <= L loop
            if Buf (K) = Byte (Character'Pos ('.')) then
               if Idx > 4 or else Oct > 255 then
                  return False;
               end if;
               IP (Idx) := Byte (Oct);
               Idx := Idx + 1;
               Oct := 0;
            elsif Buf (K) >= Byte (Character'Pos ('0'))
              and then Buf (K) <= Byte (Character'Pos ('9'))
            then
               Oct := Oct * 10 + Natural (Buf (K) - Byte (Character'Pos ('0')));
               if Oct > 255 then
                  return False;
               end if;
            else
      ...
      function Real_Client_IP
        (Head : Head_Buffer; H_End : SEO; Peer : Rate_Limiter.IP_Addr)
         return Rate_Limiter.IP_Addr
      is
         I : SEO := 1;
         Found : Rate_Limiter.IP_Addr := [others => 0];
      begin
         --  only trust forwarding headers from a loopback peer (cloudflared)
         if not Is_Loopback (Peer) then
            return Peer;
         end if;
         while I <= H_End loop
            declare
               Line_End : constant SEO :=
                 Http_Parser.Find_Byte (Head, I, H_End, 10);
               Colon : SEO;
               VStart, VEnd : SEO;
            begin
               exit when Line_End = 0;
               Colon := Http_Parser.Find_Byte
                 (Head, I, Line_End - 1, Byte (Character'Pos (':')));
               if Colon /= 0 then
                  VStart := Colon + 1;
                  while VStart <= Line_End - 1
                    and then (Head (VStart) = 32
                              or else Head (VStart) = Byte (Character'Pos (ASCII.HT)))
                  loop
                     VStart := VStart + 1;
                  end loop;
                  VEnd := Line_End - 1;
                  if VEnd >= VStart and then Head (VEnd) = 13 then
                     VEnd := VEnd - 1;
                  end if;
                  while VEnd >= VStart
                    and then (Head (VEnd) = 32
                              or else Head (VEnd) = Byte (Character'Pos (ASCII.HT)))
                  loop
                     VEnd := VEnd - 1;
                  end loop;
                  if VStart <= VEnd then
                     if Http_Parser.Name_Matches
                       (Head, I, Colon - 1, "cf-connecting-ip")
                     then
                        if Parse_IPv4 (Head, VStart, VEnd, Found) then
                           return Found;
                        end if;
                     elsif Http_Parser.Name_Matches
                       (Head, I, Colon - 1, "x-forwarded-for")
                     then
                        declare
                           C : SEO := VStart;
                        begin
                           while C <= VEnd
                             and then Head (C) /= Byte (Character'Pos (','))
                           loop
                              C := C + 1;
                           end loop;
                           if Parse_IPv4 (Head, VStart, C - 1, Found) then
                              return Found;
                           end if;
                        end;
                     end if;
      ...
      procedure Handle_One
        (Client : Socket_Type;
         ...
         CGuard : in out Conn_Guard;
         Real_IP : out Rate_Limiter.IP_Addr;
         Per_IP_Added : out Boolean)
      is
         Head : Head_Buffer;
         CR : Reader;
         ...
         begin
            loop
               ...
               begin
                  ...
                  if not Per_IP_Added then
                     declare
                        Real : constant Rate_Limiter.IP_Addr :=
                          Real_Client_IP (Head, H_End, Peer);
                        IP_Ok : Boolean;
                     begin
                        Real_IP := Real;
                        CGuard.Try_Add_Per_IP (Real, Cfg.Max_Per_IP, IP_Ok);
                        if not IP_Ok then
                           Send_Error (C_Link, Errors.E_Unavailable, 0);
                           return;
                        end if;
                        Per_IP_Added := True;
                     end;
                  ...
                  declare
                     Allowed : Boolean;
                  begin
                     --  global (all-IPs) throughput gate first: cheap and
                     --  protects the per-IP table from flood churn
                     RGuard.Check_Global
                       (Rate_Limiter.Token_Count (Cfg.Global_RPS),
                        Rate_Limiter.Token_Count (4 * Cfg.Global_RPS),
                        Allowed);
                     if not Allowed then
                        Send_Error
                          (C_Link, Errors.E_Too_Many_Requests,
                           RGuard.Global_Retry_After
                             (Rate_Limiter.Token_Count (Cfg.Global_RPS)));
                        return;
                     end if;
                  end;
                  ...
                  begin
                     ...
                     if not Allowed then
                        Send_Error
                          (C_Link, Errors.E_Too_Many_Requests,
                           RGuard.Retry_After
                             (Real_IP, Rate_Limiter.Token_Count (Cfg.Rate)));
                        return;
                     end if;
                  end;
                  Forward_Request
      ...
      is
         ...
         task body Handler is
            ...
            Target : Sock_Addr_Type;
            Cfg    : Config_Type;
            Real_IP : Rate_Limiter.IP_Addr := [others => 0];
            Per_IP_Added : Boolean := False;
         begin
            accept Start
              (C : Socket_Type;
               P : Rate_Limiter.IP_Addr;
            ...
            Handle_One (Client, Peer, Target, Cfg, RGuard, CGuard,
                        Real_IP, Per_IP_Added);
            begin
            ...
            Close_Socket (Client);
            CGuard.Remove_Total;
            if Per_IP_Added then
               CGuard.Remove_Per_IP (Real_IP);
            end if;
         exception
            when E : others =>
      ...
      begin
         ...
         begin
            ...
            --  4096 backlog so a connection flood drains through accept()
            --  instead of piling up in SYN-RECV and stalling the origin
            --  tunnel's dials (kernel connlimit caps concurrency on top)
            Listen_Socket (Listener, 4096);
         exception
            when E : others =>
         ...
         Log ("listening on 0.0.0.0:" & Trim_Int (Listen_Port)
              ...
              & " global=" & Trim_Int (Cfg.Global_RPS) & "/s"
         ...
         loop
            ...
            begin
               ...
               if Status /= Completed then
                  ...
                  CGuard.Try_Add_Total (Cfg.Max_Total, Ok);
                  if not Ok then
